...
function scene:create( event )
	...
	print( success )
	success = GBCDataCabinet.set("Scores", "score", 0)
		if (success) then
	...
	local imageFile = "maps/level1.png"
	...
	physics.addBody( rearWheel, "dynamic",
	    { density=1, friction=8, bounce=0.2, filter={ groupIndex=-1 }, radius=15 }
	)
	...
	local collisionLine = display.newImageRect( "images/invisibleLine.png", 15, 1 )
...
--ADD Sounds and Musics-------------------------------------------------------------------------------------------
	...
	function frameUpdate()
	    ...
	    if(difference>=widthCar) then 
	        ...
	        updateText()
	        if (totalDistance >= levelCellDistance) then
		        ...
		        composer.gotoScene( "scenes.level2", { effect = "fade", time = 200 } )
